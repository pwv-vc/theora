# cli-design
- Store tunable runtime parameters (e.g., compileConcurrency) in .kb/config.json with a sensible default, so they persist per-KB; CLI flags override config for one-off runs. Confidence: 0.75
...
- Add targeted regeneration sub-commands or flags for individual pipeline stages (e.g., `kb compile --concepts-only`) so users can re-run just one stage without reprocessing everything from scratch. Confidence: 0.70
